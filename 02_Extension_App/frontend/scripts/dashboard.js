...
/**
 * Load hardware statistics from storage
 */
...
function updateHardwareCounters(permissions) {
  ...
  Object.entries(counters).forEach(([type, element]) => {
    if (element && permissions[type]) {
      element.textContent = permissions[type].count || 0;
    }
  });
}

/**
 * Update hardware activity log
 */
function updateHardwareActivityLog(activityLog) {
  ...
  const activityHTML = activityLog.map(item => {
...
/**
 * Update toggle states from storage
 */
...
async function handleHardwareToggle(permissionType, shouldBlock) {
  try {
    ...
    if (response.success) {
      console.log(`[HardwareControls] ${permissionType} ${shouldBlock ? 'blocked' : 'allowed'}`);
